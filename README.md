...
Bot connects over TLS and sends framed messages: RegisterDownload / RegisterUpload (session + filename), relay replies with PortAlloc (port). Then file bytes are sent as Data frames until EOF. Same frame format is used by the fileshare bot; keep both repos in sync if you change the protocol.
